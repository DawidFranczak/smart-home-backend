...
from communication_protocol.rfid_message import check_uid_response
...
from lamp.consumer_utils import lamp_turn_on_request
...
class RouterConsumer(AsyncWebsocketConsumer):
    ...
    async def check_uid_request(self, data: DeviceMessage):
        device = await sync_to_async(Rfid.objects.get)(mac=data.device_id)
        result = await check_uid(device, data.payload["uid"])
        response = check_uid_response(data.device_id, data.message_id, result)
        await self.send(text_data=json.dumps(response.to_json()))
        if result == CheckUIDResponse.ACCESS_DENIED:
            return
        controlled_lamp = await sync_to_async(lambda: device.controlled_lamp)()
        request = await lamp_turn_on_request(controlled_lamp)
        print("request", request)
        await self.send(text_data=json.dumps(request.to_json()))
